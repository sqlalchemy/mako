...
class LookupTest(unittest.TestCase):
    ...
    def test_multi_directory_lookup(self):
        """test that all the directories are used when looking for
        templates."""
        tl = lookup.TemplateLookup(directories=[
            # does not have a `index.html` template
            os.path.join(template_base, "othersubdir"),
            # has the `index.html` template
            os.path.join(template_base, "subdir"),
        ])
        t = tl.get_template("/index.html")
        assert result_lines(t.render()) == [
            "this is sub index",
    ...
    def test_multi_directory_lookup_with_conflicting_names(self):
        """test that the presence of a directory with the same name as the
        template does not result in a lookup failure when a template with the
        same name exists in another lookup directory.
        """

        tl = lookup.TemplateLookup(directories=[
            # Contains the directory `foo.html`
            os.path.join(template_base, "subdir"),
            # Contains the template file `foo.html`
            os.path.join(template_base, "othersubdir"),
        ])
        t = tl.get_template("/foo.html")
        assert result_lines(t.render()) == []
